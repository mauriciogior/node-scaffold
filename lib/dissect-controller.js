(function()
{
	...
	DissectController.prototype =
	{
		...
		getAll : function()
		{
			...
			{
				if(this.fields[i].type == "{Ref}")
				{
					buf += "			.populate('" + this.fields[i].name + "')\n";
				}
				else if(this.fields[i].type == "[{Ref}]")
				{
					buf += "			.populate('" + this.fields[i].name + "s')\n";
				}
			}
		...
		create : function()
		{
			...
			for(var i in this.fields)
			{
				...
				{
					buf += " * @param " + this.fields[i].model + " " + this.fields[i].name + "\n";
				}
			}
			...
			if(relationships > 0)
			{
				...
				for(var i=0; i<this.fields.length; i++)
				{
					...
					{						
						...
						buf += "						path: '" + this.fields[i].name + "',\n";
						buf += "						model: '" + this.fields[i].model + "',\n";

						if(counter == relationships)
						{
							buf += "					}";
						}
						else
				...
				buf += "					" + this.nameCap + "\n";
				buf += "					.populate(" + this.name + ", options, function(err, retData)\n";
				buf += "					{\n";
				buf += "						" + this.name + " = retData;\n\n";
				buf += "						callback();\n";
				buf += "					});\n";
